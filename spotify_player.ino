...
setJpgScale()
COLOR_SAMPLE_MAX - not in use curr
SPI_Frequency in User_Setup.h

*/
#include <TJpg_Decoder.h> // jpeg decoder library
#include <Preferences.h> // persist spotify refresh to NVS flash
#include <FS.h>
...
public:
    ...
    bool refreshAuth(){
        ...
        if (httpResponseCode == HTTP_CODE_OK) {
            ...
            if (doc.containsKey("refresh_token")) {
                refreshToken = String((const char*)doc["refresh_token"]);
                persistRefreshToken();
                Serial.println("[auth] refresh token was rotated by Spotify, saved the new one"); 
            }
    ...
    bool tryAutoLogin(){
        Preferences prefs;
        prefs.begin("spotify", true); // read-only
        String saved = prefs.getString("refreshToken", "");
        prefs.end();
        if (saved.length() == 0) {
            Serial.println("[auth] no saved refresh token - manual login required");
            return false;
        }
        Serial.println("[auth] found a saved refresh token, attempting silent login");
        refreshToken = saved;
        return refreshAuth();
    }
...
private:
    ...
    void persistRefreshToken(){
        Preferences prefs;
        prefs.begin("spotify", false); // read-write
...
void setup(){
    ...
    // Try to resume a previous session before falling back to the manual
    // browser login flow. If this succeeds, accessTokenSet is already true,
    // so loop() will go straight into normal playback polling and the HTTP
    // server below will shut itself down on the very first iteration (see
    // the accessTokenSet check near the top of loop()).
    if (spotifyConnection.tryAutoLogin()) {
        Serial.println("[auth] auto-login succeeded, skipping manual Spotify login");
    } else {
        Serial.println("[auth] visit the IP above in a browser to log in to Spotify");
    } 
